...
def is_main_process():
    return (not torch_dist.is_available()) or (not torch_dist.is_initialized()) or torch_dist.get_rank() == 0
...
def parse_option():
    ...
    parser.add_argument('--output', default='./exclude/output', type=str, metavar='PATH',
...
def main(config, args):
    ...
    # print("RANK env:", os.environ.get("RANK"),
    #   "LOCAL_RANK:", os.environ.get("LOCAL_RANK"),
    #   "WORLD_SIZE:", os.environ.get("WORLD_SIZE"))
    # print("dist:", dist.is_available(), dist.is_initialized())
    # print("validate rank =", dist.get_rank(), "world_size =", dist.get_world_size())
    # exit()
    run = None
    if is_main_process():
        run = wandb.init(
        ...
        name=f"Classification-{config.MODEL.NAME}",
    ...
    if config.MODEL.PRETRAINED and (not config.MODEL.RESUME):
        ...
        acc1, acc5, loss = validate_eva(config, data_loader_val, model)
        logger.info(f"Accuracy of the network on the {len(dataset_val)} test images: {acc1:.1f}%")
        if model_ema is not None:
            acc1_ema, acc5_ema, loss_ema = validate_eva(config, data_loader_val, model_ema.ema)
            logger.info(f"Accuracy of the network ema on the {len(dataset_val)} test images: {acc1_ema:.1f}%")
    ...
    if args.enable_preload:
        # see https://github.com/triton-lang/triton/issues/3864
        for _ in data_loader_train:
            break
        for _ in data_loader_val:
            break

    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        data_loader_train.sampler.set_epoch(epoch)
        train_one_epoch(run, config, model, criterion, data_loader_train, optimizer, epoch, mixup_fn, lr_scheduler,
        ...
        acc1, acc5, loss = validate(ema, run, epoch, config, data_loader_val, model)
        logger.info(f"Accuracy of the network on the {len(dataset_val)} test images: {acc1:.1f}%")
        ...
        if model_ema is not None:
            ema=True
            acc1_ema, acc5_ema, loss_ema = validate(ema, run, epoch, config, data_loader_val, model_ema.ema)
...
def train_one_epoch(run, config, model, criterion, data_loader, optimizer, epoch, mixup_fn,
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            eta_td = datetime.timedelta(seconds=int(etas))
            if is_main_process():
                run.log({"train/loss": float(loss_meter.val),
                        "train/loss_avg": float(loss_meter.avg),
                        "train/grad_norm": float(norm_meter.val),
                        "train/grad_norm_avg": float(norm_meter.avg),
                        "train/loss_scale": float(scaler_meter.val),
                        "train/loss_scale_avg": float(scaler_meter.avg),
                        "lr": float(lr), "wd": float(wd),
                        "train/eta": str(eta_td),
                        "train/eta_seconds": int(etas),
                        })
...
@torch.no_grad()
def validate(ema, run, epoch, config, data_loader, model):
    criterion = torch.nn.CrossEntropyLoss()
    model.eval()
    ...
    for idx, (images, target) in enumerate(data_loader):
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            if is_main_process():
                if ema==True:
                    run.log({
                        "test_ema/loss": float(loss_meter.avg),
                        "test_ema/acc1": float(acc1_meter.avg),
                        "test_ema/acc5": float(acc5_meter.avg),
                        "test_ema/time": float(batch_time.avg),
                        "test_ema/mem_mb": float(memory_used),
                        "epoch": int(epoch),
                    }, step=int(epoch))
                else:
                    run.log({
                        "test/loss": float(loss_meter.avg),
                        "test/acc1": float(acc1_meter.avg),
                        "test/acc5": float(acc5_meter.avg),
                        "test/time": float(batch_time.avg),
                        "test/mem_mb": float(memory_used),
                        "epoch": int(epoch),
                    }, step=int(epoch))
...
@torch.no_grad()
def validate_eva(config, data_loader, model):
    criterion = torch.nn.CrossEntropyLoss()
    model.eval()
